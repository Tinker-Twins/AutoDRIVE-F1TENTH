...
It is expected that you have ```ros-$ROS_DISTRO-desktop``` installed (e.g., `$ROS_DISTRO = noetic`). The additional dependencies you must install are:
...
You can install them by running:
    ...
    sudo apt-get install ros-$ROS_DISTRO-tf2-geometry-msgs ros-$ROS_DISTRO-ackermann-msgs ros-$ROS_DISTRO-joy ros-$ROS_DISTRO-map-server

The full list of dependencies can be found in the ```package.xml``` file.

### Installation

To install the simulator package, clone the repo and copy the `f1tenth_sim` directory into the `src` space of your `catkin` workspace:

    cd AutoDRIVE-F1TENTH/ROS\ 1\ Packages/
    cp -r f1tenth_sim ~/catkin_ws/src
...
To instantly move the car to a new state publish [Pose](http://docs.ros.org/$ROS_DISTRO/api/geometry_msgs/html/msg/Pose.html) messages to the ```/pose``` topic. This can be useful for scripting the car through a series of automated tests.

The simulated lidar is published to the ```/scan``` topic as [LaserScan](http://docs.ros.org/$ROS_DISTRO/api/sensor_msgs/html/msg/LaserScan.html) messages.
